...
# Lista de recetas
recetas = []
recetaPrueba = Receta("Jonatan", "Sandwitch Americano", "Elaboracion de un Sandwitch americano con ingredientes frescos.", "Pan , Jamon, Queso, Lechuga, Tomate",
                      "En un sartén disponer las lonchas de tocineta cortadas por la mitad y cocinar a fuego bajo hasta que suelten su grasa y queden crocantes. Cuando estén listas pasar por papel absorbente para eliminar el exceso de grasa. Aparte en otro sartén, derretir un poco de mantequilla y añadir las rodajas de pan, dorar por ambos lados. ", "20 min", "https://www.hogar.mapfre.es/media/2019/02/sandwich-americano.jpg")
...
def buscarUsuario(user):
    for usuariox in usuarios:
        if usuariox.usuario == user:
            datosUsuario = [usuariox.nombre,
                            usuariox.apellido, usuariox.contrasena]
            return datosUsuario
...
def buscarReceta(tit):
    for receta in recetas:
        if receta.titulo == tit:
            datosReceta = [receta.autor, receta.titulo, receta.resumen,
                           receta.ingredientes, receta.procedimiento, receta.tiempo, receta.imagen]
            return datosReceta
    ...
    if request.method == 'POST':
        if "user" in session:
            ...
            if userMod == currentUser and contrasenaMod == contrasena2Mod:
                ...
                usuarios.append(
                    Usuario(userMod, contrasenaMod, nombreMod, apellidoMod))
            ...
            if usuarioExistente(userMod) == False:
                ...
                deleteUsuario(currentUser)
                session.pop("user", None)
                newUser = Usuario(userMod, contrasenaMod,
    ...
    if request.method == 'POST':
        ...
        if usuarioExistente(usuario) == False and contrasena == contrasena2:
            administradores.append(
                Usuario(usuario, contrasena, nombre, apellido))
            error = None
            confirm = 'Se a registrado el usuario'
...
def RegistrarReceta():
    if "user" in session:
        ...
        if request.method == 'POST':
            ...
            recetas.append(Receta(autor, titulo, resumen,
                           ingredientes, procedimiento, tiempo, imagen))
...
def DescargarRecetasPDF():
    ...
    rendered = render_template(
        'recetasPDF.html', recetas=recetas, usuario=usuario)
...
def DescargarUsuariosPDF():
    ...
    rendered = render_template('usuariosPDF.html', usuarios=usuarios,
                               administradores=administradores, usuario=usuario)
